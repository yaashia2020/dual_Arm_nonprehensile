...
class ExplicitReferenceGovernor:
    def __init__(self, robust_delta_tau_, kappa_tau_, 
        ...
        # ----------------------------------------------------------------------
        # Create prediction context and store handles to important bodies
        # ----------------------------------------------------------------------
        self.context_pred = self.plant_pred.CreateDefaultContext()

        # End-effectors of the two robots (adjust names if needed)
        # ----------------------------------------------------------------------
        # Create prediction context and store handles to important bodies
        # ----------------------------------------------------------------------

        # End-effectors of the two robots
        # ----------------------------------------------------------------------
        # Create prediction context and store handles to important bodies
        # ----------------------------------------------------------------------
        self.context_pred = self.plant_pred.CreateDefaultContext()

        # End-effector contact bodies (rubber pads)
        try:
            panda1_id = self.plant_pred.GetModelInstanceByName("panda")
            panda2_id = self.plant_pred.GetModelInstanceByName("panda_1")

            self.body_E1 = self.plant_pred.GetBodyByName("rubber_pad", panda1_id)
            self.body_E2 = self.plant_pred.GetBodyByName("rubber_pad", panda2_id)
            print("✅ Using 'rubber_pad' as contact body for both grippers.")

        except Exception as e:
            print(f"⚠️ Could not find 'rubber_pad' in plant. Falling back to panda_link7. ({e})")
            try:
                # Use panda_link7 for each specific robot instance
                panda1_id = self.plant_pred.GetModelInstanceByName("panda")
                panda2_id = self.plant_pred.GetModelInstanceByName("panda_1")

                self.body_E1 = self.plant_pred.GetBodyByName("panda_link7", panda1_id)
                self.body_E2 = self.plant_pred.GetBodyByName("panda_link7", panda2_id)
                print("✅ Using 'panda_link7' as contact body for both grippers.")

            except Exception as e2:
                print(f"⚠️ Fallback failed. Using world body. ({e2})")
                self.body_E1 = self.plant_pred.world_body()
                self.body_E2 = self.plant_pred.world_body()
        ...
        # # Movable box body
        # try:
        #     self.body_box = self.plant_pred.GetBodyByName("movable_box")
        # except Exception as e:
        #     print(f"⚠️ Could not find 'movable_box'. Using world as fallback. ({e})")
        #     self.body_box = self.plant_pred.world_body()
        # ----------------------------------------------------------------------
    ...
    def get_qv(self, q, dq, tau, q_r, q_v, box_position=None):
        ...
        # Get world poses of each end-effector and box from Drake
        # ✅ New code (Option 1: use real box position from main simulation)

        # ✅ Use the plant_context (which is already being updated throughout the ERG)
        self.plant_pred.SetPositions(self.plant_context, q)
        self.plant_pred.SetVelocities(self.plant_context, dq)

        X_W_E1 = self.plant_pred.EvalBodyPoseInWorld(self.plant_context, self.body_E1)
        X_W_E2 = self.plant_pred.EvalBodyPoseInWorld(self.plant_context, self.body_E2)

        if box_position is not None:
            p_box = np.array(box_position).reshape(3)
        else:
            p_box = np.zeros(3)

        # Run grasp optimization (pass full poses)
        self.compute_grasp_optimization(p_box, X_W_E1, X_W_E2)
    ...
    ###########################################
    #   Grasp Force Optimization (Drake)
    ###########################################

    def compute_grasp_optimization(self, p_box, X_W_E1, X_W_E2):
        from pydrake.all import MathematicalProgram, Solve
        import numpy as np

        # --- Geometry setup ---
        p_E1 = X_W_E1.translation()
        p_E2 = X_W_E2.translation()
        n1_W = X_W_E1.rotation().col(0)  # normal from left pad
        n2_W = -X_W_E2.rotation().col(0)  # normal from right pad

        # Wrench to balance
        m = 2.0
        g = 9.81
        h_c = np.array([0, 0, -m * g, 0, 0, 0])
        ...
        # Internal squeeze direction
        V = np.concatenate([n1_W, n2_W]).reshape(6, 1)
        ...
        prog.AddConstraint(np.linalg.norm(f1 - n1_W * (f1 @ n1_W)) <= mu * (f1 @ n1_W))
        prog.AddConstraint(np.linalg.norm(f2 - n2_W * (f2 @ n2_W)) <= mu * (f2 @ n2_W))

        # Positivity
        prog.AddConstraint((f1 @ n1_W) >= 0)
        prog.AddConstraint((f2 @ n2_W) >= 0)
        ...
        if result.is_success():
            print("✅ Grasp OK, z_d =", result.GetSolution(z_d))
        else:
            print("❌ Infeasible grasp config.")

    # def compute_grasp_optimization(self, p_box, X_W_E1, X_W_E2):
    #     """
    #     Simplified loop force-closure optimization:
    #     - Only considers normal forces along the rubber pad normals.
    #     - Ensures enough normal force to prevent slip under gravity.
    #     - Ignores moments (assumes symmetric grasp).
    #     """
    #
    #     import numpy as np
    #
    #     # ------------------------------------------------------------------
    #     # Extract poses and normals (±Y direction from URDF)
    #     # ------------------------------------------------------------------
    #     R_W_E1 = X_W_E1.rotation().matrix()
    #     R_W_E2 = X_W_E2.rotation().matrix()
    #     n1_W = -R_W_E1[:, 1]  # left pad normal (+Y toward box)
    #     n2_W = R_W_E2[:, 1]  # right pad normal (-Y toward box)
    #
    #     # ------------------------------------------------------------------
    #     # Box parameters
    #     # ------------------------------------------------------------------
    ...
    #
    #     # Required normal force per gripper to prevent slip
    #     fN_min = (m * g) / (2 * mu)
    #     fN = 1.1 * fN_min  # add 10% safety margin
    #
    #     # ------------------------------------------------------------------
    #     # Compute resulting contact forces in world frame
    #     # ------------------------------------------------------------------
    #     f1 = fN * n1_W
    #     f2 = fN * n2_W
    #
    #     # Resulting net force on box
    #     F_total = f1 + f2 + np.array([0, 0, -m * g])
    #
    #     # ------------------------------------------------------------------
    #     # Diagnostics
    #     # ------------------------------------------------------------------
    #     slip_margin = (m * g) / (2 * mu * fN)
    #     print("✅ Normal-force closure computed.")
    #     print(f"  Normal directions: n1={n1_W}, n2={n2_W}")
    #     print(f"  Normal force per pad = {fN:.3f} N  (min required {fN_min:.3f} N)")
    #     print(f"  Slip margin = {slip_margin:.3f}  (<1 means safe)")
    #     print(f"  Net force residual = {np.linalg.norm(F_total):.3e} N")
    #
    #     # Return stacked vector (for compatibility)
    ...
    # def compute_grasp_optimization(self, p_box, X_W_E1, X_W_E2):
    #     """
    #     Force-closure grasp optimization using *actual* gripper poses
    #     (URDF-corrected for Franka's rubber_pad geometry).
    #
    #     Each gripper applies a contact force f_i in its local contact frame.
    #     The optimization finds minimal normal forces along the gripper pad normals
    #     required to prevent slipping under gravity.
    #
    #     Args:
    #         p_box: np.array(3,), current box center position in world frame
    #         X_W_E1, X_W_E2: RigidTransform of each gripper in world frame
    #     """
    #     from pydrake.solvers import MathematicalProgram, Solve
    #     import numpy as np
    #
    #     def skew(v):
    #         """Return the 3×3 skew-symmetric matrix for cross products."""
    #         return np.array([
    #             [0, -v[2], v[1]],
    #             [v[2], 0, -v[0]],
    #             [-v[1], v[0], 0]
    ...
    #
    #     # ------------------------------------------------------------------
    #     # Contact geometry: actual world positions of gripper pads
    #     # ------------------------------------------------------------------
    #     p_E1 = X_W_E1.translation()
    #     p_E2 = X_W_E2.translation()
    #     r1 = p_E1 - p_box
    ...
    #     # For Franka rubber_pad: surface normal = -Y of the pad frame
    #     # Left gripper pushes +Y toward box, right gripper pushes -Y.
    #     R_W_E1 = X_W_E1.rotation().matrix()
    #     R_W_E2 = X_W_E2.rotation().matrix()
    #     n1_W = -R_W_E1[:, 1]
    #     n2_W = R_W_E2[:, 1]
    #
    #     # ------------------------------------------------------------------
    #     # Decision variables: contact forces at both pads
    #     # f = [f1x,f1y,f1z,f2x,f2y,f2z]
    #     # These are expressed in WORLD frame
    #     # ------------------------------------------------------------------
    #     f = prog.NewContinuousVariables(6, "f")
    #
    #     mu = 0.5  # friction coefficient
    #     m = 2.0  # box mass [kg]
    #     g = 9.81
    #     h_c = np.array([0, 0, -m * g, 0, 0, 0])  # desired net wrench (gravity compensation)
    #
    #     # ------------------------------------------------------------------
    #     # Normal and friction constraints
    #     # ------------------------------------------------------------------
    #     # Project force along normal: fN = nᵀf
    #     f1N = np.dot(n1_W, f[0:3])
    #     f2N = np.dot(n2_W, f[3:6])
    #
    #     # Normal forces must be compressive (push on box)
    #     prog.AddLinearConstraint(f1N >= 0)
    #     prog.AddLinearConstraint(f2N >= 0)
    #
    #     # Friction cone constraints (‖f_t‖ ≤ μ f_N)
    #     def tangential_force_squared(fx, fy, fz, n):
    #         n = n / np.linalg.norm(n)
    #         ft = np.array([fx, fy, fz]) - np.dot(np.array([fx, fy, fz]), n) * n
    #         return np.dot(ft, ft)
    #
    #     prog.AddConstraint(tangential_force_squared(f[0], f[1], f[2], n1_W) <= (mu * f1N) ** 2)
    #     prog.AddConstraint(tangential_force_squared(f[3], f[4], f[5], n2_W) <= (mu * f2N) ** 2)
    #
    #     # ------------------------------------------------------------------
    #     # Build grasp matrix W (maps contact forces → net wrench on box)
    #     # ------------------------------------------------------------------
    #     W = np.zeros((6, 6))
    #     W[0:3, 0:3] = np.eye(3)
    #     W[3:6, 0:3] = skew(r1)
    ...
    #     W[3:6, 3:6] = skew(r2)
    #
    #     # Static equilibrium: W f = h_c
    #     prog.AddLinearEqualityConstraint(W, h_c, f)
    #
    #     # ------------------------------------------------------------------
    #     # Objective: minimize total normal effort (weighted)
    #     # ------------------------------------------------------------------
    #     Q = np.diag([1, 0.01, 0.01, 1, 0.01, 0.01])
    #     prog.AddQuadraticCost(Q, np.zeros(6), f)
    #
    #     # Initial guess: balanced squeeze
    #     fN_guess = (m * g) / (2 * mu)
    #     prog.SetInitialGuess(f, np.array([0, fN_guess, m * g / 2, 0, -fN_guess, m * g / 2]))
    #
    #     # ------------------------------------------------------------------
    #     # Solve
    #     # ------------------------------------------------------------------
    ...
    #         print("❌ Grasp optimization failed (infeasible).")
    #         print(f"  h_c={h_c}, r1={r1}, r2={r2}")
    #         total_energy = np.sum(np.square(result.GetSolution(f)))
    #         print(f"Total energy: {total_energy}")
    #         return None
    #
    #     # ------------------------------------------------------------------
    ...
    #     f1 = f_opt[:3]
    #     f2 = f_opt[3:]
    #     residual = np.linalg.norm(W.dot(f_opt) - h_c)
    #     total_energy = np.sum(np.square(f_opt))
    #
    #     # Slip margin: ratio of tangential to friction limit
    #     def slip_margin(f_vec, n):
    #         n = n / np.linalg.norm(n)
    #         fN = np.dot(f_vec, n)
    #         ft = f_vec - fN * n
    #         return np.linalg.norm(ft) / (mu * fN + 1e-9)
    #
    #     slip1 = slip_margin(f1, n1_W)
    #     slip2 = slip_margin(f2, n2_W)
    #
    #     print("✅ Force-closure optimization solved.")
    #     print(f"  f1 = {f1}, f2 = {f2}")
    #     print(f"  Residual equilibrium norm = {residual:.3e}")
    #     print(f"  Slip margin (1,2) = ({slip1:.2f}, {slip2:.2f})  (<1 means stable)")
    #     print(f"  Total normal effort = {f1N + f2N:.3f} N")
    #     print(f"  Contact offsets: r1 = {r1}, r2 = {r2}")
