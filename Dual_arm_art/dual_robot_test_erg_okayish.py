...
class DualRelaxedIKBoxTracker(LeafSystem):
    ...
    def solve_ik(self, context, discrete_state):
        ...
        # --- Target positions (slide along x toward box) ---
        target_position1 = [x_b - half_extents[0] - pad_offset, y_b, 0.4]  # left arm
        target_position2 = [x_b + half_extents[0] + pad_offset, y_b, 0.4]  # right arm


        # target_position1 = [0.2, 0, 0.5]  # left arm
        # target_position2 = [0.2, 0, 0.5]  # right arm


        # --- Orientations (locked from init) ---
        orientation1 = self.ee1_initial_quat
        orientation2 = self.ee2_initial_quat
